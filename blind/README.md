# **Project Title: Generative Modeling of Full-Atom Protein Conformations (LD-FPG Implementation)## 🧬 Overview**
...
The pipeline consists of three main stages:1. Autoencoder Training (chebnet_blind.py): A Chebyshev Graph Neural Network (ChebNet) based autoencoder (HNO Encoder + Decoder2 MLP) is trained to learn a compressed latent representation (a global pooled embedding h0​) of protein conformations.

2\. Latent Diffusion Model Training (new_diff.py): A Denoising Diffusion Probabilistic Model (DDPM) is trained on the distribution of these pooled latent embeddings h0​.

3\. Structure Reconstruction (chebnet_diff.py): New latent embeddings h0gen​ sampled by the trained diffusion model are decoded back into full-atom 3D protein coordinates using the trained Decoder2 MLP.
...
1. **Stage 1: ChebNet Autoencoder (**chebnet_blind.py**)**
    ...
    - **Pooling:** For the "Blind Pooling" strategy, these atom-wise embeddings are globally pooled (using AdaptiveAvgPool2d) across all atoms to create a single, compact latent vector h0​ for each conformation.
    - **Decoder (Decoder2):** An MLP that takes the pooled latent vector h0​ and a conditioner (e.g., latent representation of a reference structure, Zref​) to reconstruct the full-atom Cartesian coordinates.
...
2. **Stage 2: Latent Diffusion Model (**new_diff.py**)**
    - Takes the h0​ embeddings (generated and saved by chebnet_blind.py) as input.
    - Trains a DDPM (either MLP-based or Conv2D-based denoiser) to learn the manifold of these compact latent representations.
    - Generates new h0gen​ samples by reversing the diffusion process.
3. **Stage 3: Structure Reconstruction from Diffused Latents (**chebnet_diff.py**)**
    - Uses the trained Decoder2 MLP from Stage 1.
    - Takes the novel h0gen​ samples (generated by new_diff.py) and the same conditioner used during Decoder2 training.
...
- **Purpose:** Train the HNO encoder and the Decoder2 MLP. This stage learns to compress protein structures into a global pooled latent representation (h0​) and reconstruct them.
- **Configuration (**param.yaml**):**
  ...
  - decoder2_settings:
    ...
    - output_height, output_width: Dimensions for the AdaptiveAvgPool2d in blind pooling. The product (output_height \* output_width) defines the dimension of the pooled latent vector h0​.
...
- **Execution:**  
    ...
    <br/>Add --debug for verbose logging.
- **Key Inputs:**
  - param.yaml
...
- **Key Outputs:**
  ...
  - latent_reps/pooled_embedding.h5: Pooled latent embeddings (h0​) of the input MD data. **This is the primary input for Step 2.** (Dataset key: pooled_embedding).
...
- **Purpose:** Train a DDPM on the distribution of pooled latent embeddings (h0​) generated in Step 1. This model will then be able to generate new h0gen​ samples.
...
- **Execution:**  
    ...
    <br/>\# For a specific experiment from a grid search (e.g., if 'run_mode: grid_search')  
    \# python new_diff.py --config param_diff.yaml --exp_idx 1  
    <br/>Use --instance_id for parallel grid search execution. Add --debug for verbose logs.
- **Key Inputs:**
  - param_diff.yaml
...
- **Key Outputs (within** parameters.output_dir**, e.g.,** latent_reps/**):**
  ...
  - generated_embeddings_exp&lt;N&gt;.h5: Newly generated pooled latent embeddings (h0gen​) from experiment &lt;N&gt;. **This is the primary input for Step 3.** (Dataset key: generated_embeddings). The &lt;N&gt; corresponds to the global experiment index if running a grid search.
...
- **Stage 1 Output (**chebnet_blind.py**) -> Stage 2 Input (**new_diff.py**):**
  - latent_reps/pooled_embedding.h5 (containing h0​ for all input frames) is the crucial input for training the diffusion model.
...
- **Stage 2 Output (**new_diff.py**) -> Stage 3 Input (**chebnet_diff.py**):**
  - The selected latent_reps/generated_embeddings_exp&lt;N&gt;.h5 (containing new h0gen​ samples).
