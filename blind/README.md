...
The pipeline consists of three main stages:1.  Autoencoder Training (chebnet_blind.py): A Chebyshev Graph Neural Network (ChebNet) based autoencoder (HNO Encoder + Decoder2 MLP) is trained to learn a compressed latent representation (a global pooled embedding h 
0
​
 ) of protein conformations.
2.  Latent Diffusion Model Training (new_diff.py): A Denoising Diffusion Probabilistic Model (DDPM) is trained on the distribution of these pooled latent embeddings h 
0
​
 .
3.  Structure Reconstruction (chebnet_diff.py): New latent embeddings h 
0
gen
​
  sampled by the trained diffusion model are decoded back into full-atom 3D protein coordinates using the trained Decoder2 MLP.

🛠️ Methodology Highlights
The core methodology implemented in this repository involves:

Stage 1: ChebNet Autoencoder (chebnet_blind.py)
...
Pooling: For the "Blind Pooling" strategy, these atom-wise embeddings are globally pooled (using AdaptiveAvgPool2d) across all atoms to create a single, compact latent vector h 
0
​
  for each conformation.

Decoder (Decoder2): An MLP that takes the pooled latent vector h 
0
​
  and a conditioner (e.g., latent representation of a reference structure, Z 
ref
​
 ) to reconstruct the full-atom Cartesian coordinates.
...
Stage 2: Latent Diffusion Model (new_diff.py)

Takes the h 
0
​
  embeddings (generated and saved by chebnet_blind.py) as input.
...
gen
​
  samples by reversing the diffusion process.

Stage 3: Structure Reconstruction from Diffused Latents (chebnet_diff.py)
...
gen
​
  samples (generated by new_diff.py) and the same conditioner used during Decoder2 training.
...
Purpose: Train the HNO encoder and the Decoder2 MLP. This stage learns to compress protein structures into a global pooled latent representation (h 
0
​
...
output_height, output_width: Dimensions for the AdaptiveAvgPool2d in blind pooling. The product (output_height * output_width) defines the dimension of the pooled latent vector h 
0
​
 .
...
latent_reps/pooled_embedding.h5: Pooled latent embeddings (h 
0
​
 ) of the input MD data. This is the primary input for Step 2. (Dataset key: pooled_embedding).
...
Purpose: Train a DDPM on the distribution of pooled latent embeddings (h 
0
​
 ) generated in Step 1. This model will then be able to generate new h 
0
gen
​
  samples.
...
generated_embeddings_exp<N>.h5: Newly generated pooled latent embeddings (h 
0
gen
​
 ) from experiment <N>. This is the primary input for Step 3. (Dataset key: generated_embeddings). The <N> corresponds to the global experiment index if running a grid search.
...
latent_reps/pooled_embedding.h5 (containing h 
0
​
  for all input frames) is the crucial input for training the diffusion model.
...
The selected latent_reps/generated_embeddings_exp<N>.h5 (containing new h 
0
gen
​
  samples).
